...
bit roundoff = 0;
int adcVal=0;
int initVal=0;totalBit=0,mvolts=0,thv=0;
int weight;
...
void timer0_ISR (void) interrupt 1
{
	...
	totalBit=totalBit+adcVal;
	counter++;
	
	if(counter==10)
	{
		counter=0;
		totalBit = (totalBit*4)/9;
		weight = (int)totalBit;
		LCD_CmdWrite(0x80);
		LCD_StringWrite("Weight",6);
		LCD_CmdWrite(0xCB);
		
		LCD_DataWrite(totalBit/1000 + '0');
		
		temp = totalBit%1000;
		LCD_DataWrite(temp/100 + '0');
		
		temp = temp%100;
		LCD_DataWrite(temp/10 + '0');
